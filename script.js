...
function formatInputValue(value, type = 'requests') {
  const monthlyValue = value * 30; // Convert daily to monthly
  const formattedValue = formatNumberWithSuffix(monthlyValue);
  
  switch(type) {
      case 'requests':
      case 'operations':
          return `${formattedValue}/month`;
      case 'storage':
          return `${value} GB`;
      case 'time':
          return `${value} ms`;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  inputs.forEach(input => {
      const inputType = getInputType(input.id);
      
      // Add event listeners for input formatting
      input.addEventListener('input', function() {
          if (this.value < 0) this.value = 0;
          
          // Show formatted value in a span next to the input
          let formatSpan = this.nextElementSibling;
          if (!formatSpan || !formatSpan.classList.contains('formatted-value')) {
              formatSpan = document.createElement('span');
              formatSpan.classList.add('formatted-value');
              this.parentNode.insertBefore(formatSpan, this.nextSibling);
          }
          
          if (this.value && this.value > 0) {
              const formattedValue = formatInputValue(parseFloat(this.value), inputType);
              formatSpan.textContent = `= ${formattedValue}`;
...
function getInputType(inputId) {
  if (inputId.includes('storage')) return 'storage';
  if (inputId.includes('cpu')) return 'time';
  if (inputId.includes('class')) return 'operations';
  return 'requests';
};
